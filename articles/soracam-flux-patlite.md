...
SORACOM Fluxの「1. モーション検知 webhook」アクションで設定したAPI ChannelのWebhookURLを取得します。

1. SORACOM Fluxの該当アクションを選択
2. 「API Channel」の詳細を確認し、WebhookURLをコピー

#### 2. ソラカメの通知設定

1. [SORACOMユーザーコンソール](https://console.soracom.io/)にログイン
2. [メニュー] → [ソラコムクラウドカメラサービス] → [通知設定] をクリック
3. 「ソラカメ通知オプションのご利用について」画面が表示された場合は、内容を理解したうえで [理解して利用する] をクリック
4. 通知設定画面で [デバイスのモーション検知/サウンド検出の通知] の[通知を設置する]をクリックする
![alt text](/images/soracam-flux-patlite/1751588114843.png)

#### 3. モーション検知通知の設定

1. **通知先の設定**:
   - Webhook のタブを選択し、[+Webhook通知先を追加] をクリック
   ![alt text](/images/soracam-flux-patlite/1751588268433.png)
   - 手順1で取得したSORACOM FluxのWebhookURLを設定
   - HTTPメソッド: `POST`
   - HTTPヘッダー: デフォルトのまま
...
#### 4. 通知対象デバイスの設定
1. 通知対象デバイスの設定:
   - 通知設定画面で [通知対象デバイスを設定する] をクリック
   - 一覧からモーション検知を行うカメラを選択し、通知対象のトグルをオンにします。


#### 5. 送信されるペイロードの設定
[通知メッセージ設定]の[モーション検知/サウンド検出の設定]の項目で、トグルを有効化、そのあと[メッセージ内容を設定する]のwebhookのタブを選択します。
前の手順で設定した通知先のwebhookメッセージを選択して[webhookメッセージを編集する]をクリックします。
